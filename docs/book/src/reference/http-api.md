...
`crates/varve-server/src/{api.rs,http/{mod.rs,handlers.rs}}`.
...
never leaks internal detail: `internal` errors are generic even when the underlying GQL
referenced something sensitive (`invalid_requests_negotiation_timeout_and_internal_errors_are_stable`
asserts the literal string `secret_storage_credential` from a failing query never appears in the
response body).
