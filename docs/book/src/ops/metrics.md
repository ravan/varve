...
Reference for everything Varve exposes at `GET /metrics`
(Prometheus text format, `PrometheusMetrics` in
`crates/varve-server/src/metrics/mod.rs`), the stable `tracing` span names
emitted across the write/read/follower paths, and the coordination objects and
failover runbook for the `[coordinator]` subsystem (spec §12). Every name
below is copied verbatim from source.

## 1. Metrics

All metrics are served on the writer/query-node HTTP surface at `/metrics`.
Some metric names ending in `_total` are implemented as a Prometheus
`IntGauge`/`IntGaugeVec` rather than a `Counter`, noted explicitly below.
...
`rate()`/`increase()` behave correctly. One caveat: because they are a
`Gauge` type rather than `Counter`, Prometheus counter-reset detection does
not apply, so after a process restart the gauge drops to 0 and `rate()`
briefly reads negative before it corrects. Only `varve_http_requests_total`
is a true `IntCounterVec`.
...
| `varve_applied_tx_id` | gauge | (none) | Latest applied transaction id on this node | `varve_applied_tx_id` |
| `varve_applied_log_position` | gauge | (none) | Latest applied log position on this node | `varve_applied_log_position` |
| `varve_manifest_watermark` | gauge | (none) | Current manifest watermark | `varve_manifest_watermark` |
| `varve_follower_healthy` | gauge (0/1) | (none) | 1 while the follower loop has no error; 0 once `follower_error` is set | `min_over_time(varve_follower_healthy[5m]) == 0`, alert on follower stalls |
| `varve_log_head_position` | gauge | (none) | Latest known log head (writer publishes its own durable position, lag 0 by construction; a follower/query node publishes `max(last-read+1, watermark seen in gap check)`, or the jumped cursor on a fence jump) | `varve_log_head_position` |
| `varve_log_lag_records` | gauge | (none) | **Follower lag**: records between this node's applied position and the (locally known) log head | `varve_log_lag_records` per node, alert if sustained > 0 on a query node |
| `varve_txs_committed_total` | gauge, monotone-by-construction (mirrors an engine `AtomicU64`, `_total`-named but not a `Counter` type) | (none) | Committed transactions | **ingest rate**: `rate(varve_txs_committed_total[1m])` |
| `varve_events_committed_total` | gauge, monotone-by-construction | (none) | Committed events (finer-grained than tx count) | `rate(varve_events_committed_total[1m])` |
| `varve_commit_failures_total` | gauge, monotone-by-construction | (none) | Pre-durability append failures | `increase(varve_commit_failures_total[15m]) > 0`, alert |
| `varve_flush_blocks_total` | gauge, monotone-by-construction | (none) | Successful block flushes | `rate(varve_flush_blocks_total[5m])` |
| `varve_flush_failures_total` | gauge, monotone-by-construction | (none) | Failed block flushes | `increase(varve_flush_failures_total[15m]) > 0`, alert |
| `varve_compaction_runs_total` | gauge, monotone-by-construction | (none) | Compaction runs | `rate(varve_compaction_runs_total[1h])` |
| `varve_backpressure_rejections_total` | gauge, monotone-by-construction | (none) | `try_execute_as` rejections under backpressure (429s) | `rate(varve_backpressure_rejections_total[1m])`, sustained > 0 means writers are saturated |
| `varve_live_rows` | gauge | (none) | Unflushed rows across all graphs | `varve_live_rows` |
...
| `varve.compact` | `compact` (`writer.rs`) | (none) | Wraps an entire compaction run |
...
| `varve.query.parse` | query entrypoint (`db.rs`) | (none) | Synchronous GQL parse (entered span, no async work) |
| `varve.query.plan` | query entrypoint (`db.rs`) | (none) | Query planning, including each leg of a UNION |
...
- `endpoint` is required; the `PrometheusMetricsFactory`/`OtlpMetricsFactory`
...
with `tls_built_in_native_certs(false)`, so TLS certificate validation checks
...
is not in that bundle, the push fails TLS verification silently: the
...
- `cas-failover` also requires the storage backend's conditional-put
  ...
  `CasUnsupported` below). Verified against both MinIO (probe
  ...
  rejecting it, so the gate correctly refuses with an actionable error).
