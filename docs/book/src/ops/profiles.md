...
topology: pinned Garage plus a distroless writer plus 2 query nodes, driven over HTTP, verifying
both query nodes agree on a basis read before tearing everything down.
...
referenced by node `_id` and not verified; a dangling edge is durable but never
...
**Memory limit:** a compaction job merges all of its input events in
...
Defaults come from `crates/varve-engine/src/db.rs` and `crates/varve-storage/src/
{cache,disk}.rs`; every one is overridable in `varve.toml`.
