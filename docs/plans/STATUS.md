...
- **2026-07-12 (slice 11 Task 6, fuzz manifest/block-meta/event decoders):** manifest fuzzing (`BlockManifest::from_wire`, trie-key parsing, `TrieCatalog::from_manifests`) found ZERO crashes across two 120s runs (~4.2M/5.5M execs) -- this boundary is confirmed panic-free on arbitrary bytes. `block_meta`/`events` fuzzing (`varve_index::decode_meta`/`decode_events`, both `StreamReader`-based Arrow IPC readers) each crashed within seconds. **KNOWN UPSTREAM ISSUE (arrow-rs 58.3.0, not preventable by our own input validation -- exhausted: `skip_validation` defaults to validation-ON already, no builder-level opt-in exists for `StreamReader` unlike `FileReaderBuilder`, and the "safe" free function `try_schema_from_flatbuffer_bytes` shares the same buggy internals):** (1) `arrow_ipc::convert::fb_to_schema` (`convert.rs:198`) unconditionally `.unwrap()`s `fb.fields()`, panicking when a Schema flatbuffer omits its fields vector -- artifact `fuzz/regressions/block_meta/fb-to-schema-unwrap.bin`. (2) `RecordBatchDecoder::next_buffer` -> `arrow_buffer::Buffer::slice_with_length` panics ("offset ... cannot exceed ... length") on malformed buffer offset/length metadata even with validation enabled -- artifact `fuzz/regressions/block_meta/buffer-slice-panic.bin`. (3) `arrow_ipc::convert::get_data_type` (`convert.rs:401` and sibling branches) `panic!()`s on out-of-range enum discriminants for IntervalUnit/TimeUnit/etc -- artifact `fuzz/regressions/events/interval-unit-panic.bin`. Per the task's findings policy, NOT `catch_unwind`'d (would mask, not fix); this is a genuine unfixable-at-our-boundary robustness gap -- any caller decoding untrusted/possibly-corrupted meta or event bytes (e.g. recovery from a tampered S3 object) can be panicked, not just handed an `Err`. Flagged as a real DoS-surface concern for a future arrow-rs upgrade or a from-scratch schema-validation shim; not blocking Slice 11 per the brief's explicit escalation clause.
